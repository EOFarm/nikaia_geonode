...
RUN mkdir -p /usr/src/geonode_nikaia
...
COPY src /usr/src/geonode_nikaia/
WORKDIR /usr/src/geonode_nikaia

COPY src/wait-for-databases.sh /usr/bin/wait-for-databases
RUN chmod +x /usr/bin/wait-for-databases
RUN chmod +x /usr/src/geonode_nikaia/tasks.py \
    && chmod +x /usr/src/geonode_nikaia/entrypoint.sh
